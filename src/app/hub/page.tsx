    ...
    >
      ...
      <div className="relative h-10 overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-center h-full text-xs text-slate-400">No stories right now</div>
        ) : (
          <div
          ...
          >
            {tickerItems.map((item, idx) => (
              <a key={`${item.id}-${idx}`} href={item.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2.5 shrink-0 group">
                <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded-full ${CATEGORY_COLORS[item.category] || CATEGORY_COLORS["General AI"]}`}>{item.category}</span>
                <span className="text-[12px] font-semibold text-slate-700 group-hover:text-violet-600 transition-colors max-w-[380px] truncate">{item.title}</span>
                <span className="text-[9px] text-slate-400 flex items-center gap-0.5"><TrendingUp className="w-2.5 h-2.5" />{item.points}</span>
